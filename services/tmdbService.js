...
async function getWatchProviders(id, type) {
  if (!env.TMDB_API_KEY) throw new Error("TMDB_API_KEY missing");
  const endpoint = type === "series" ? "tv" : "movie";
  const res = await fetch(
...
async function discover({ type, genres, providers }) {
  if (!env.TMDB_API_KEY) {
    ...
    const genreNames = idsToNames(genres);
  ...
  // Expande ids virtuais (negativos) para conjuntos de ids TMDB reais
