...
import { PROVIDERS_MAP, STREAMING_SERVICES } from "@/config/providers";
...
async function discover({ type, genres, providers }) {
  // Debug log para verificar uso da chave no cliente
  console.log(`[TMDB Client] Buscando: Type=${type}, Genres=${genres.length}, Providers=${providers.length}`);
  console.log(`[TMDB Client] API Key configurada? ${env.TMDB_API_KEY ? "SIM" : "NÃO"}`);

  if (!env.TMDB_API_KEY) {
    console.warn("[TMDB] Chave de API não encontrada. Usando modo offline (Mock Data).");
    console.warn("Para usar a API real, certifique-se de ter NEXT_PUBLIC_TMDB_API_KEY configurado.");
